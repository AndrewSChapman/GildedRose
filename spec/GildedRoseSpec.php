...
use App\Item;
use App\GildedRose;
use App\QualityStrategy\QualityStrategyFactory;
use App\QualityStrategy\AppreciationQualityStrategy;
use App\QualityStrategy\DeprecationQualityStrategy;
use App\QualityStrategy\BackstagePassQualityStrategy;
use App\QualityStrategy\StaticQualityStrategy;
use App\ProductName;

describe('Gilded Rose', function () {
    describe('QualityStrategyFactory', function () {
        context('StrategyCreation', function () {
            it('correctly creates an appreciation strategy for aged brie', function () {
                $item = new Item(ProductName::AGED_BRIE, 10, 5);
                $factory = new QualityStrategyFactory();
                $strategy = $factory->getQualityStrategy($item);

                expect($strategy)->toBeAnInstanceOf(AppreciationQualityStrategy::class);
            });
            it('correctly creates a a backstage strategy for back stage item', function () {
                $item = new Item(ProductName::BACKSTAGE_PASS, 10, 5);
                $factory = new QualityStrategyFactory();
                $strategy = $factory->getQualityStrategy($item);

                expect($strategy)->toBeAnInstanceOf(BackstagePassQualityStrategy::class);
            });
            it('correctly creates a static quality strategy for sulfuras', function () {
                $item = new Item(ProductName::SULFURAS, 10, 5);
                $factory = new QualityStrategyFactory();
                $strategy = $factory->getQualityStrategy($item);

                expect($strategy)->toBeAnInstanceOf(StaticQualityStrategy::class);
            });
            it('correctly creates a depreciation strategy for all other items', function () {
                $item = new Item('Some Item', 10, 5);
                $factory = new QualityStrategyFactory();
                $strategy = $factory->getQualityStrategy($item);

                expect($strategy)->toBeAnInstanceOf(DeprecationQualityStrategy::class);
            });
        });
    });
    describe('next day', function () {
        context('normal Items', function () {
            it('updates normal items before sell date', function () {
                $gr = new GildedRose(new QualityStrategyFactory(), [new Item('normal', 10, 5)]);
                $gr->nextDay();
                expect($gr->getItem(0)->quality)->toBe(9);
                expect($gr->getItem(0)->sellIn)->toBe(4);
            });
            it('updates normal items on the sell date', function () {
                $gr = new GildedRose(new QualityStrategyFactory(), [new Item('normal', 10, 0)]);
                $gr->nextDay();
            ...
            it('updates normal items with a quality of 0', function () {
                $gr = new GildedRose(new QualityStrategyFactory(), [new Item('normal', 0, 5)]);
                $gr->nextDay();
